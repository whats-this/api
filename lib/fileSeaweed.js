...
module.exports = ({ key, contentType, body }) => {
  ...
  return http.get({ path: weedAssign.path, host: weedAssign.hostname, port: weedAssign.port || 80 }).promise()
    .then(res => {
      swAssignRes = res;
      return collectJson(res);
    })
    .then(body => {
      if (swAssignRes.statusCode > 299) {
        throw new Error(
          'unexpected response code from SeaweedFS assign request, got ' + swAssignRes.statusCode + ', body ' + JSON.stringify(body)
        );
      }
      return body;
    })
