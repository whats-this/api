  ...
  Promise.resolve()
    ...
    .then(resp => new Promise((resolve, reject) => {
      ...
      db.query({
        ...
        text: 'SELECT id, email, username, is_blocked, is_admin FROM users WHERE id=$1'
    ...
    .then(resp => {
      ...
      req.userId = resp.rows[0].id;
      req.email = resp.rows[0].email;
      req.isAdmin = resp.rows[0].is_admin;
      req.username = resp.rows[0].username;
    })
    .then(cb)
    .catch(err => {
