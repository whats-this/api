...
module.exports = (req, res, cb) => {
  let token = req.headers['authorization'] || url.parse(req.url, true).query['key'] || url.parse(req.url, true).query['apikey'];
  ...
  DynamoDB.scan({
    ...
    if (data.Items.length === 0) {
      // Database query didn't match, therefore there are no users with this token
      res.writeHead(401, 'Unauthorized', JSON.stringify({
        success: false,
        errorcode: 401,
